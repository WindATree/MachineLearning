...
# import label_propagation2 as LPA  # TODO: 这里需要引入你封装好的LPA函数 (标签传播算法)
# from label_propagation import interpolate_features
...
adata.write('../data/newdataset.h5ad')  # 将数据存储到文件中

# k1初始化为1.0，表示在LPA过程中标签传播的权重
k1 = 1.0
...
while True:
    ...
    # adata = sc.read_h5ad('../data/dataset.h5ad')
    # sc.pp.neighbors(adata, n_neighbors=8, method='umap', metric="correlation")
    adata.write('../data/newdataset.h5ad')  # 将数据存储到文件中
    # 读取HDF5文件 'dataset1.h5ad'
    with h5py.File('../data/newdataset.h5ad', 'r') as f:
        # 从文件中获取连接矩阵的数据
    ...
    # 再次读取HDF5文件，获取观察数据
    with h5py.File('../data/newdataset.h5ad', 'r') as h5file:
    ...
    #         sc.pp.neighbors(adata, n_neighbors=8, method='umap', metric="correlation")
    #     elif k1 <= 0.6:
    #         sc.pp.neighbors(adata, n_neighbors=12, method='umap', metric="correlation")
    #     elif k1 <= 0.4:
    #         sc.pp.neighbors(adata, n_neighbors=15, method='umap', metric="correlation")
    #     elif k1 <= 0.2:
    #         sc.pp.neighbors(adata, n_neighbors=15, method='umap', metric="correlation")

    for _ in range(10):  # 循环100次进行评估
        # 初始化LPA输入矩阵X，大小为mat的行数×行数
        ...
        # 进行标签传播
        # start1 = time.time()
        LPA.dataProcess(y_label, y_new, k1, (1 - k1), 0)  # 处理数据
        # end1 = time.time()
        # print(start1-end1)
        # interpolate_features(X, range_radius=1)
        LPA.labelPropagation(X, y_new, y_pred, y_res, 0.5, 10000)
        # LPA.labelPropagationGNN(X, y_label, y_pred, y_res)
        # LPA.opt_labelPropagation(X, y_new, y_pred, y_res, 0.5, 1000,1e-5)  # 执行LPA算法
        # LPA.opt_labelPropagation(X, y_new, y_pred, y_res)
...
# 将所有实验结果保存为TSV文件
df.to_csv("OptLPA_mistake.tsv", sep='\t', header=True, index=True)
