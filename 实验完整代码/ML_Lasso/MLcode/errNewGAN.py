...
import NewGAN as GAN  # TODO: 这里需要引入你封装好的GAN函数 (标签传播算法)
...
while True:
    for _ in range(50):  # 循环100次进行评估
        ...
        nparr_mat=np.array(mat)
        y_pred_fzy = GAN.label_propagation(nparr_mat,dense_matrix, y_new.v)
        y_res_fzy=y_pred_fzy
        # 计算执行时间
...
# 将所有实验结果保存为TSV文件
df.to_csv("GAN2_mistake.tsv", sep='\t', header=True, index=True)
