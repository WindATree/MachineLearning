...
import GNN as GNN  # TODO: 这里需要引入你封装好的GNN函数 (标签传播算法)
# import pairpotlpa as GNN
...
# k1初始化为1.0，表示在GNN过程中标签传播的权重
k1 = 1.0

# 开始循环，逐步减少k1的值来测试算法的鲁棒性
while True:
    for _ in range(100):  # 循环100次进行评估
        # 初始化GNN输入矩阵X，大小为mat的行数×行数
        ...
        y_res = GNN.mat(mat.shape[0], len(val))

        # 进行标签传播
        # start1 = time.time()
        GNN.dataProcess(y_label, y_new, k1, (1 - k1), 0)  # 处理数据
        # end1 = time.time()
        # print(start1-end1)
        GNN.labelPropagation_GNN(X, y_new, y_pred, y_res)  # 执行GNN算法
...
# 将所有实验结果保存为TSV文件
df.to_csv("GNNmistake.tsv", sep='\t', header=True, index=True)
