...
import randomforest as randomforest  # TODO: 这里需要引入你封装好的randomforest函数 (标签传播算法)
# import pairpotlpa as randomforest
...
# k1初始化为1.0，表示在randomforest过程中标签传播的权重
k1 = 1.0

# 开始循环，逐步减少k1的值来测试算法的鲁棒性
while True:
    for _ in range(5):  # 循环100次进行评估
        # 初始化randomforest输入矩阵X，大小为mat的行数×行数
        X = randomforest.matCoo(mat.shape[0], mat.shape[0])
        for i in range(len(data)):
            X.append(rows[i], cols[i], data[i])

        # 初始化标签矩阵y_label和y_new，用于存储标签信息
        y_label = randomforest.mat(mat.shape[0], len(val))
        y_new = randomforest.mat(mat.shape[0], len(val))
        ...
        y_pred = randomforest.mat(mat.shape[0], len(val))
        y_res = randomforest.mat(mat.shape[0], len(val))

        # 进行标签传播
        # start1 = time.time()
        randomforest.dataProcess(y_label, y_new, k1, (1 - k1), 0)  # 处理数据
        # end1 = time.time()
        # print(start1-end1)
        # y_pred_fzy,y_res_fzy=randomforest.labelPropagation(csr,y_new.v, y_pred.v, y_res.v)  # 执行randomforest算法
        # y_pred_fzy, y_res_fzy = randomforest.labelPropagation(csr,y_new.v)
        rf_label_propagation = randomforest.RandomForestLabelPropagation(n_estimators=100, max_iter=100)
        y_pred_fzy, y_res_fzy = rf_label_propagation.labelPropagation(dense_matrix, y_new.v)
...
# 将所有实验结果保存为TSV文件
df.to_csv("randomforest_mistake.tsv", sep='\t', header=True, index=True)
